...
maxclique: $(SRC)/main.cpp $(COMPONENT_OUTPUT_DIR)/libhpx_incumbent_component.so $(COMPONENT_OUTPUT_DIR)/libhpx_workqueue_component.so $(BUILD)/DimacsParser.o
	$(CXX) $(CXXFLAGS) -o $@ $< $(BUILD)/DimacsParser.o `pkg-config --cflags --libs hpx_application` -I$(COMPONENT_DIR) -DHPX_APPLICATION_NAME=maxclique -L$(COMPONENT_OUTPUT_DIR) -lhpx_incumbent_component -lhpx_iostreams -lhpx_workqueue_component
